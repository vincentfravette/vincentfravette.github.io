...
! function () {
	...
	window.tram = function (t) {
		...
		if (I.transition) {
			var $ = I.timing.dom;
			if (j.style[$] = d["ease-in-back"][0], !j.style[$])
		...
		var L = e.frame = function () {
			...
			P = f(N, function (t, e) {
				...
				t.init = function () {
					e.init.apply(this, arguments), this.current || (this.current = {}, J.perspective && U.perspective && (this.current.perspective = U.perspective, Y(this.el, this.name, this.style(this.current)), this.redraw()))
				}, t.set = function (t) {
		...
		I.transform && (Q.transform = [P], J = {
			...
			skewX: [_],
			skewY: [_]
		}), I.transform && I.backface && (J.z = [k, "translateZ"], J.rotateZ = [_], J.scaleZ = [b], J.perspective = [x]);
		var V = /ms/,
			K = /s|\./;
		return t.tram = e
	}(window.jQuery);
	var e = {},
		...
		r = t(function (t) {
			...
			f.env.safari = /safari/.test(_) && !E && !A;
			var j;
			...
			f.load = function (t) {
				...
				t = t || {}, k = !0, m.triggerHandler("__wf_destroy"), null != t.domready && (x = t.domready), b.each(d, o), f.resize.off(), f.scroll.off(), f.redraw.off(), h = [], p = [], "pending" === C.state() && l()
...
//		}), 
			...
			e.define("edit", t.exports = function (t, n, i) {
				...
				return g ? m() : p.search ? (/[?&](edit)(?:[=&?]|$)/.test(p.search) || /\?edit$/.test(p.href)) && m() : d.on(v, r).triggerHandler(v), f
			})
		}), t(function (t) {
			/*!
			...
			e.define("forms", t.exports = function (t, n) {
				...
				function r(e, n) {
					...
					r.handler = null, r.redirect = i.attr("data-redirect"), E.test(s) ? r.handler = f : s || (v ? r.handler = l : A())
				}
		...
		u = (t(function (t) {
			...
			e.define("ix", t.exports = function (t, i) {
				...
				function a() {
					var n = t("[data-ix]");
					n.length && (n.each(c), n.each(s), j.length && (e.scroll.on(l), setTimeout(l, 1)), z.length && e.load(f), C.length && setTimeout(d, M))
				}
					...
					if (c) {
						...
						l && (w.style(a, c.style), i.each(l, function (t) {
							...
							if ("load" !== s) {
								...
								if ("hover" === s) return a.on("mouseenter" + y, i), a.on("mouseleave" + y, r), void(E = E.add(a));
								if ("scroll" !== s) {
									var l = I[s];
									if (l) {
										var f = a.closest(l);
							...
							} else t.preload && !_ ? z.push(i) : C.push(i)
						}))
					}
				}
				...
				function l() {
					for (var t = b.scrollTop(), e = b.height(), n = j.length, i = 0; i < n; i++) {
						var r = j[i],
							o = r.el,
							a = r.trigger,
				...
				return w.init = function (t) {
					...
					g = !0, E.each(c), e.scroll.off(l), n.async(), j = [], z = [], C = []
				}, w.ready = function () {
				...
				r.build = function () {
					...
					if (t !== b) {
						...
						return b = t, x.show(), c(e.html && w(e.width, e.height) || e.url, function (r) {
							function o() {
								x.hide(), t === b ? (m(y.arrowLeft, "inactive", t <= 0), m(y.arrowRight, "inactive", t >= E.length - 1), y.view ? (k(y.view).add("opacity .3s").start({
									opacity: 0
	...
	t(function (t) {
		...
		e.define("navbar", t.exports = function (t, i) {
			...
			function s(e, n) {
				...
				r || (r = t.data(n, M, {
					...
					el: i,
					config: {}
				})), r.menu = i.find(".w-nav-menu"), r.links = r.menu.find(".w-nav-link"), r.dropdowns = r.menu.find(".w-dropdown"), r.button = i.find(".w-nav-button"), r.container = i.find(".w-container"), r.outside = m(r), r.el.off(M), r.button.off(M), r.menu.off(M), f(r), T ? (c(r), r.el.on("setting" + M, d(r))) : (l(r), r.button.on("tap" + M, p(r)), r.menu.on("click" + M, "a", v(r))), g(e, n)
			}
			...
			function m(e) {
				return e.outside && j.off("tap" + M, e.outside), i.debounce(function (n) {
					if (e.open) {
			...
			function b(t, n) {
				if (!t.open) {
					t.open = !0, t.menu.addClass($), t.links.addClass(q), t.button.addClass(I);
					var i = t.config;
					...
					var r = y(t),
						...
						s = t.el.height(),
						u = t.el[0];
					if (g(0, u), L.intro(0, u), e.redraw.up(), T || j.on("tap" + M, t.outside), !n) {
			...
			function x(t, e) {
				function n() {
					t.menu.height(""), E(t.menu).set({
						x: 0,
						y: 0
					}), t.menu.removeClass($), t.links.removeClass(q), t.overlay && t.overlay.children().length && (D.length ? t.menu.insertAfter(D) : t.menu.prependTo(t.parent), t.overlay.attr("style", "").hide()), t.el.triggerHandler("w-close")
				}
				if (t.open) {
					t.open = !1, t.button.removeClass(I);
					var i = t.config;
					if (("none" === i.animation || !E.support.transform || i.duration <= 0) && (e = !0), L.outro(0, t.el[0]), j.off("tap" + M, t.outside), e) return E(t.menu).stop(), void n();
			...
			O.ready = O.design = O.preview = function () {
				T = z && e.env("design"), k = t(document.body), (_ = j.find(M)).length && (_.each(s), r(), o())
			}, O.destroy = function () {
		...
		e.define("slider", t.exports = function (t, i) {
			function r() {
				(_ = z.find(M)).length && (_.filter(":visible").each(u), E = null, O || (o(), a()))
			}
			...
			function u(e, n) {
				...
				if (r || (r = t.data(n, M, {
						...
						el: i,
						config: {}
					})), r.mask = i.children(".w-slider-mask"), r.left = i.children(".w-slider-arrow-left"), r.right = i.children(".w-slider-arrow-right"), r.nav = i.children(".w-slider-nav"), r.slides = r.mask.children(".w-slide"), r.slides.each($.reset), E && (r.maskWidth = 0), !j.support.transform) return r.left.hide(), r.right.hide(), r.nav.hide(), void(O = !0);
				r.el.off(M), r.left.off(M), r.right.off(M), r.nav.off(M), c(r), T ? (r.el.on("setting" + M, m(r)), v(r), r.hasTimer = !1) : (r.el.on("swipe" + M, m(r)), r.left.on("tap" + M, f(r)), r.right.on("tap" + M, d(r)), r.config.autoplay && !r.hasTimer && (r.hasTimer = !0, r.timerCount = 1, p(r))), r.nav.on("tap" + M, "> div", m(r)), C || r.mask.contents().filter(function () {
			...
			function g(e, n) {
				...
				var c = e.offsetX || 0,
					...
					b = "opacity " + g + "ms " + m,
					y = "transform " + g + "ms " + m;
				if (T || (d.each($.intro), p.each($.outro)), n.immediate && !E) return j(d).set(f), void i();
				if (e.index !== e.previous) {
					if ("cross" === v) {
						var x = Math.round(g - g * r.crossOver),
							k = Math.round(g - x);
						return b = "opacity " + x + "ms " + m, j(h).set({
							visibility: ""
						}).add(b).start({
							opacity: 0
						}), void j(d).set({
			...
			function y(e) {
				...
				r && (r = parseFloat(r) + "px");
				for (var o = 0; o < e.pages; o++) n = t(I), e.nav.hasClass("w-num") && n.text(o + 1), null != r && n.css({
					"margin-left": r,
					"margin-right": r
				}), i.push(n);
...
Webflow.require('ix').init([{
	...
	"slug": "review-arrow-left",
	"name": "Review Arrow Left",
	"value": {
		"style": {
			"x": "-60px",
			"y": "0px",
	...
	"slug": "review-arrow-right",
	"name": "Review Arrow Right",
	"value": {
		"style": {
			"opacity": 0,
			"x": "60px",
		...
		"triggers": [{
			...
			"stepsA": [{
				"opacity": 1,
				"transition": "transform 900ms ease 0, opacity 900ms ease 0",
				"x": "0px",
				"y": "0px",
				"z": "0px"
			}],
	...
	"value": {
		...
		"triggers": [{
			...
			"stepsA": [{
				"display": "block",
				"opacity": 1,
				"transition": "opacity 300ms ease 0, transform 200 ease 0"
			}],
			"stepsB": [{
				"opacity": 0,
				"transition": "opacity 300ms ease 0, transform 200 ease 0"
	...
	"value": {
		...
		"triggers": [{
			"type": "hover",
			"selector": ".instagram-photo-block",
			"stepsA": [{
				"opacity": 0.7,
				"transition": "opacity 300ms ease 0"
			}],
	...
	"value": {
		...
		"triggers": [{
			...
			"type": "hover",
			"selector": ".work-card-image",
			"descend": true,
			"preserve3d": true,
			"stepsA": [{
				"opacity": 0.7,
				"transition": "transform 450ms ease 0, opacity 450ms ease 0",
				"scaleX": 1,
				"scaleY": 1,
				"scaleZ": 1
			}],
			...
			"stepsB": [{
				"opacity": 0,
				"transition": "opacity 450ms ease 0, transform 450ms ease 0",
				"scaleX": 1.5,
				"scaleY": 1.5,
				"scaleZ": 1
			}]
	...
	"value": {
		...
		"triggers": [{
			...
			"stepsA": [{
				...
				"transition": "transform 1200ms ease 0",
				"x": "100%",
				"y": "0px",
				"z": "0px"
			}],
	...
	"slug": "side-header-line",
	"name": "Side Header Line",
	"value": {
		"style": {
			"opacity": 0,
			"width": "1px"
